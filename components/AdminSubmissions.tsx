...
interface AdminSubmissionsProps {
  ...
  onBulkAction?: (submissionIds: string[], action: 'approve' | 'reject') => void
  filter: 'all' | 'pending' | 'approved' | 'rejected'
}

export default function AdminSubmissions({ submissions, onAction, onBulkAction, filter }: AdminSubmissionsProps) {
  ...
  const autoGeneratedSubmissions = pendingSubmissions.filter(s => s.submitterEmail === 'system@auto-scraper.com')
  ...
  const handleBulkAction = (action: 'approve' | 'reject') => {
    if (onBulkAction && selectedSubmissions.size > 0) {
      onBulkAction(Array.from(selectedSubmissions), action)
      setSelectedSubmissions(new Set())
      setShowBulkActions(false)
    }
  }

  const isAutoGenerated = (submission: Submission) => {
    return submission.submitterEmail === 'system@auto-scraper.com'
  }

  const getAutoGeneratedBadge = (submission: Submission) => {
    if (isAutoGenerated(submission)) {
      return (
        <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-blue-500 text-white ml-2">
          🤖 Auto-Generated
        </span>
      )
    }
  ...
  return (
    <div className="space-y-4">
      ...
      {filter === 'pending' && pendingSubmissions.length > 0 && (
        <div className={cn("rounded-lg border p-4", colors.background.card, colors.border.accent)}>
            ...
            <div className="flex items-center space-x-4">
              <label className="flex items-center space-x-2">
                ...
                <span className={cn("text-sm font-medium", colors.text.primary)}>
                  Select All ({pendingSubmissions.length} pending)
                </span>
              </label>
              {autoGeneratedSubmissions.length > 0 && (
                <span className={cn("text-sm", colors.text.secondary)}>
                  🤖 {autoGeneratedSubmissions.length} auto-generated
                </span>
              )}
            </div>
            ...
            {showBulkActions && (
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  className={cn("px-3 py-1 text-sm font-medium rounded-md transition-colors", colors.button.primary)}
                >
                  Approve All
                </button>
                <button
                  onClick={() => handleBulkAction('reject')}
                  className={cn("px-3 py-1 text-sm font-medium rounded-md transition-colors", colors.button.danger)}
                >
                  Reject All
                </button>
              </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    {getAutoGeneratedBadge(submission)}
                  </div>
